...
def before_test():
	...
	setup_complete({
		...
		"bank_account": "Primary Chequeing"
	})
...
suppliers = [
	...
	("AgriTheory", "ERPNext Consulting", "Cheque", 1000.00),
	("HIJ Telecom, Inc", "Internet Services", "Cheque", 150.00),
	("Sphere Cellular", "Phone Services", "ACH/EFT", 250.00),
	("Cooperative Ag Finance", "Financial Services", "Wire Transfer", 5000.00),
]

tax_authority = [
	("Local Tax Authority", "Payroll Taxes", "Cheque", 0.00),
]
...
def create_test_data():
	...
	settings.company_account = update_account_number(company_account, 'Primary Chequeing', account_number="1201", from_descendant=False)
...
def create_bank_and_bank_account():
	...
	if not frappe.db.exists('Bank Account', 'Primary Chequeing - Local Bank'):
		bank_account = frappe.new_doc('Bank Account')
		bank_account.account_name = 'Primary Chequeing'
		...
		bank_account.cheque_number = 2500
		bank_account.save()
...
def create_employees():
	for employee_number in range(1, 13):
		...
		emp.mode_of_payment = 'Cheque' if employee_number % 3 == 0 else 'ACH/EFT'
...
def create_payroll_journal_entry():
	...
	for idx, emp in enumerate(emps):
		...
		je.append('accounts', {
			...
			'user_remark': employee_name + ' Paycheque',
			'idx': idx + 2
		})
